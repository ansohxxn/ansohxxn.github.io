...
void triangle(int len, int startRow, int startCol)
{
    ...
    if (len % 2)
        triangle(len - 1, startRow - rowNum / 2, startCol + rowNum / 2 + 1); // 홀수일 때 재귀. 이제 1 감소 후 짝수로 재귀하여 역삼각형 그려야함. 홀수면 시작점은 좌하단
    else
        triangle(len - 1, startRow + rowNum / 2, startCol + rowNum / 2 + 1); // 짝수일 때 재귀. 이제 1 감소 후 짝수로 재귀하여 역삼각형 그려야함. 짝수면 시작점은 좌상단
}
...
![image](https://user-images.githubusercontent.com/42318591/88461432-70dfd400-cede-11ea-8b2b-c8c2b72b3493.png){: width="60%" height="60%"}{: .align-center}
...
2. `재귀` 방식으로 알맞는 자리에 `*`을 <u>배열 원소로 넣는다.</u>
  1. **draw** 함수로 현재 삼각형의 <u>시작 지점으로부터 알맞는 자리에 * 를 배열에 넣어준다.</u>
  2. 길이가 1, 즉 N=1 크기의 삼각형까지 재귀했을 경우 종료
  3. 재귀! 홀수, 짝수에 따라 시작 지점, 즉 인수가 다르기 때문에 다르게 재귀 호출 해준다. 
3. 배열 원소들 쫙 출력
