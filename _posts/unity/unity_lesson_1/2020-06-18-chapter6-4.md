---
title:  "Unity Chapter 6-3. 어메이징 볼링 게임 만들기 : 랜덤 오브젝트 생성기, 카메라 추적" 
...
public class SpawnGenerator : MonoBehaviour
{
    ...
    public void Reset() // public으로 만들어야. 외부에서 이 함수를 쓸 수 있도록.
    {
        // 프롭들의 위치를 랜덤 포지션으로 다시 지정
        for (int i = 0; i < props.Count; i++) // 리스트는 Count를 사용함. 배열은 Length
        {
            props[i].transform.position = GetRandomPosition();
            props[i].SetActive(true); // 꺼져 있던 친구도 켜줌
        }
    }
}
...
public class CamFollow : MonoBehaviour
{
    ...
    private State state  // 변수임. 함수 아님. 프로퍼티 사용. 겉으로보면 변수지만 내부로 보면 함수
    {
        set
        {
            switch(value)
    ...
    private const float trackingZoomSize = 14.5f;
