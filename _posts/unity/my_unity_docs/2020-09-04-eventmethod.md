...
- **Awake 와의 차이점**
  - Awake() 👉 오브젝트가 생성될 때 최초 실행
  - Start() 👉 오브젝트가 활성화 될 때마다 1 회 실행 (OnEnable과 비슷)
- **OnEnable 와의 차이점**
...
- **Start() 와의 차이점**
  - Awake() 👉 오브젝트가 생성될 때 최초 실행
    - 오브젝트가 비활성화 됐다가 다시 활성화되도 실행되지 않는다. 그저 오브젝트가 최초로 생성될 때만 실행 됨.
      - 따라서 싱글톤 구현은 Awake() 안에서 하는 것이 좋다.
  - Start() 👉 오브젝트가 활성화 될 때마다 1 회 실행 
    - 오브젝트가 비활성화 됐다가 다시 활성화되도 또 호출 된다.
    - (OnEnable과 비슷)
